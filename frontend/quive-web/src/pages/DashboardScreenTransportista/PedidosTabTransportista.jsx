...
const PedidosTabTransportista = ({userType, userData}) => {
  ...
  const aceptarPedido = async (idSolicitud) => {
    const confirm = window.confirm("¿Estás seguro de aceptar este pedido?");
    if (!confirm) return;

    try {
      const res = await fetch("http://localhost:5000/asignaciones/asignar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id_solicitud: idSolicitud,
          id_transportista: userData.id_usuario
        })
      });
      ...
      if (res.ok) {
        ...
        // Aquí podrías actualizar la lista o redirigir
      } else {
        alert("Error: " + data.msg);
      }
    } catch (err) {
      
      alert("Error de red al aceptar el pedido");
    }
  };
...
export default PedidosTabTransportista;
